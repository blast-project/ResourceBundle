...
namespace Blast\Bundle\ResourceBundle\ORM\Mapping;
 ...
 */
class ORMUnderscoredBundleNameStrategy implements NamingStrategy
{
            ...
            $propertyName = null)
    {
        ...
        return
                $tableName
                .
                (($this->joinTableFieldSuffix && null !== $propertyName) ? '_' . $this->propertyToColumnName($propertyName,
                        $sourceEntity) : '');
    }
    ...
    private function getNamingMap(KernelInterface $kernel, array $configuration)
    {
        ...
        foreach ( $kernel->getBundles() as $bundle ) {
            if ( count($configuration['blacklist']) > 0 && in_array($bundle->getName(),
                            $configuration['blacklist']) ) {
                continue;
            }
            if ( count($configuration['whitelist']) > 0 && !in_array($bundle->getName(),
